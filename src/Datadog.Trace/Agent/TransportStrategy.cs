        ...
        public static IApiRequestFactory Get(TracerSettings settings)
        {
            ...
            switch (strategy)
            {
                ...
                case DatadogNamedPipes:
                    Log.Debug("Using {FactoryType} for trace transport, with pipe name {PipeName} and timeout {Timeout}ms.", nameof(NamedPipeClientStreamFactory), settings.TracesPipeName, settings.TracesPipeTimeoutMs);
