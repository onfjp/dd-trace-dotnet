...
namespace Datadog.Trace.Agent
{
    internal static class TransportStrategy
    {
        ...
        public static IApiRequestFactory Get(TracerSettings settings)
        {
            ...
            {
                case DatadogTcp:
                    Log.Debug("Using {FactoryType} for trace transport.", nameof(TcpStreamFactory));
                ...
                case DatadogNamedPipes:
                    Log.Debug<string, string, int>("Using {FactoryType} for trace transport, with pipe name {PipeName} and timeout {Timeout}ms.", nameof(NamedPipeClientStreamFactory), settings.TracesPipeName, settings.TracesPipeTimeoutMs);
