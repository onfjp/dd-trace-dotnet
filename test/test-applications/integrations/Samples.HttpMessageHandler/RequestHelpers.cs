...
#if NET5_0
        public static void SendHttpClientRequests(bool tracingDisabled, string url, string requestContent)
        {
            // Insert a call to the Tracer.Instance to include an AssemblyRef to Datadog.Trace assembly in the final executable
            ...
            using (var clientWithCustomHandler = new HttpClient(new CustomHandler()))
            {
                ...
                using (Tracer.Instance.StartActive("CustomHandler"))
                {
                    clientWithCustomHandler.Send(new HttpRequestMessage(HttpMethod.Get, url));
                    Console.WriteLine("Received response for clientWithCustomHandler.Get");
                }
            }
        }
#endif
